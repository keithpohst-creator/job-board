  ...
  return (
    <div className="bg-white border border-slate-200 hover:border-slate-300 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between group">
      <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {job.glassdoorRating && (
                  <span 
                    className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-emerald-50 text-emerald-800 border border-emerald-200 text-[10px] font-bold"
              ...
              <h3 
                ...
                className="text-base font-bold text-slate-900 group-hover:text-[#0952c4] cursor-pointer transition-colors line-clamp-1 mt-0.5"
              >
                {job.title}
              </h3>
            </div>
        ...
        {/* Location, Salary, Work Model, Experience, Applicants, Posting Date */}
        <div className="flex flex-wrap items-center gap-2 mb-3.5 text-xs text-slate-600">
          ...
          <span className="flex items-center gap-1 bg-blue-50/80 border border-blue-100 text-[#0952c4] px-2 py-0.5 rounded-md font-semibold">
            <Users className="w-3 h-3 text-[#0952c4]" />
            <span>{job.applicantsCount.toLocaleString()} applicants to date</span>
          </span>
          <span className="flex items-center gap-1 text-[11px] text-slate-500 bg-slate-50 border border-slate-200/80 px-2 py-0.5 rounded-md ml-auto">
            <Calendar className="w-3 h-3 text-slate-400" />
            <span>{job.postedDate}</span>
            <span className="text-slate-400 font-medium">({job.daysOpen} {job.daysOpen === 1 ? 'day' : 'days'} open)</span>
          </span>
        </div>
